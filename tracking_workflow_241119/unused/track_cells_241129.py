...
#@ File (label="Select masks file", style="file") mask_path
#@ File (label="Select the probability file", style="file") prob_path
...
imp_probs = IJ.openImage(str(prob_path))
...
f.write("track_id,spot_id,mask_id,fr,X,Y,area,mean,edge,p_interphase,p_mitosis,p_death,cell_class\n")
...
for id in model.getTrackModel().trackIDs(True):
    ...
    for spot in track:
        ...
        imp_probs.setSlice(fr)
        imp_probs.setRoi(roi)
        imp_probs.setC(2) # Interphase
        prob_interphase = IJ.getValue(imp_probs, "Mean")
        imp_probs.setC(3) # Mitosis
        prob_mitosis = IJ.getValue(imp_probs, "Mean")
        imp_probs.setC(4) # Death
        prob_death = IJ.getValue(imp_probs, "Mean")
        cell_class = getCellClass(prob_interphase, prob_mitosis, prob_death)
        
        #print(id, sid, fr, x, y, area, mean)
        f.write("%d,%d,%d,%d,%d,%d,%0.3f,%0.5f,%s,%0.5f,%0.5f,%0.5f,%s\n"%(id, sid, smean+1, fr, X, Y, area, mean, roi_at_edge, prob_interphase, prob_mitosis, prob_death, cell_class))
