...
out_dir = "/Users/u2260235/Documents/Y3 Project/tracking_workflow_241119/4_mask_track_id"
...
# Match track files to mask files, ensuring correct order
track_files = []
for basename in basenames:
    if basename in track_files_map:
        track_files.append(track_files_map[basename])
    else:
        # Handle missing track files
        print(f"Warning: No track file found for {basename}.")
        track_files.append(None)
#%%

for file_id in range(len(mask_files)): # goes through every file
    print(f"Processing file: {mask_files[file_id]}")
    masks = tf.imread(mask_files[file_id])
    
    tracks = pd.read_csv(track_files[file_id])
    ...
    savepath = out_dir + '/' + basenames[file_id] + '_masks'+ '.tiff'
    tf.imwrite(savepath, masks)
